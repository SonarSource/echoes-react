...
type SelectionCardOption = RadioOption & {
  /**
...
export interface SelectionCardsProps extends RadioButtonGroupProps {
  cardsContainerClassName?: string;
  options: SelectionCardOption[];
}

export const SelectionCards = forwardRef<HTMLDivElement, PropsWithLabels<SelectionCardsProps>>(
  (props, ref) => {
    const {
    ...
    const defaultId = `${useId()}selectiongroup`;
...
function SelectionCard(props: Readonly<SelectionCardOption>) {
  ...
  return (
    <StyledSelectionCard aria-label={inputLabel} disabled={isDisabled} value={value}>
      ...
      <StyledCardContentWrapper hasIllustration={isDefined(illustration)}>
...
const StyledCardContentWrapper = styled.div<{ hasIllustration: boolean }>`
  display: inline-flex;
  flex-direction: column;
  align-items: start;
