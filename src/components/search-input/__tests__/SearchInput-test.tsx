...
const MIN_LENGTH_MESSAGE = '(3 chars min)';
...
it('should warn when input is too short', async () => {
  ...
  expect(screen.getByRole('searchbox', { name: 'Search (3 chars min)' })).toBeInTheDocument();
...
it('should not show min length message when minLength is 1 or less', () => {
  ...
  expect(screen.queryByRole('searchbox', { name: 'Search (1 chars min)' })).not.toBeInTheDocument();
...
it('should allow to customize labels', async () => {
  const minLengthLabel = 'minimum 3 characters';
