...
const MORE_FILTER_CATEGORIES: FilterDropdownCategoryWithItems[] = [
  {
    isMultiSelect: true,
    items: [
...
function IssuesPage() {
  ...
  const [moreFilters, setMoreFilters] = useState<FilterDropdownSelectedValues>({});
  ...
  const filteredIssues = useMemo(() => {
    const typeFilters = moreFilters['issue-type'] ?? [];
    const assigneeFilters = moreFilters['assignee'] ?? [];
  ...
  const filterTags = [
    ...
    ...(status !== null && status !== 'all'
      ...
      : []),
    ...Object.keys(moreFilters).map((catKey) => (
      <>
